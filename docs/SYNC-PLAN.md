...
> **Superseded for the `TrackEntity` half — see ARCHITECTURE-REVIEW.md Tier 4.1.** That seam was drawn while the client was JSON and the server was EF Core, so "keep SQLite/EF concerns out of the shared model" was keeping a real asymmetry out. Both sides are now on one raw-SQLite layer (`Flower.Core/Persistence/Sql/`), where the seam would only duplicate SQL, so the schema and the repositories are deliberately shared instead. `SubsonicSongDto` is untouched and stays a seam: the wire shape is a published protocol, which is a different argument entirely. The client is ported; `Flower.Server` still has to follow.
