...
**Remaining work:** fold the client into the `IMusicImporter` abstraction as a user-facing settings choice; add Jellyfin as a second `IMusicImporter` backend; real-device Android download-path verification and end-to-end testing against a real peer; and, now the next initiative, build order step 4 - scaffolding the `Flower.Web` Avalonia Browser/WASM head, including deciding how a paired device's `TrustedPeerStore` signing key actually unlocks `/rest/*` access rather than just being recorded (today's OpenSubsonic auth is still the single shared admin token) - before Docker packaging + Tailscale/LettuceEncrypt docs (step 5).
