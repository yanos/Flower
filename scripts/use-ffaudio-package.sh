...
# The gitignored half of the pair. FFAudioPackage.props is tracked and holds
# the switch itself; this file holds only what is true of one machine - the
# version in force and the local feed it came from - and setting the version is
# what activates everything over there.
...
cat > "$props" <<PROPS
<Project>
  <!-- Generated by scripts/use-ffaudio-package.sh, and gitignored: a version
       and a path that mean nothing on anyone else's disk. Deleting this file
       is the whole of the undo, which is what the script's "off" mode does.
       Do not edit by hand; re-run the script instead.

       The switch this activates lives in FFAudioPackage.props, which is
       tracked and stays inert while FFAudioPackageVersion is unset. -->
  <PropertyGroup>
    ...
    <!-- Empty when this pack produced no iOS payload, which is an ordinary
         outcome of a local pack on a machine that has not cross-compiled one.
         The switch leaves the iOS heads on their checked-in framework when so,
         rather than referencing a package that was never built. -->
    <FFAudioIosPackage>$ios_payload</FFAudioIosPackage>
  </PropertyGroup>
</Project>
PROPS
...
echo "            it drives FFAudioPackage.props, which is tracked and inert without it"
echo
