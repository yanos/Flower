...
public class MusicListViewGestureTests : PinnedDataDirectory
{
    ...
    private static List<TrackRowViewModel> Rows(int count) =>
        ...
        {
            Track = new Track { Path = $"/music/{i}.mp3", Title = $"Track {i}", Album = "Album" },
        }).ToList();
    ...
    // so IsSelected never survives on its own - it is re-applied by Track.Path.
    [AvaloniaFact]
    public void MultiSelectionSurvivesSetItemsByPath()
    {
        using var h = new Harness(10);
    ...
    public void SetItemsDropsSelectedRowsNoLongerPresent()
    {
        ...
        // must not resurrect it. Only the retained path is re-applied.
