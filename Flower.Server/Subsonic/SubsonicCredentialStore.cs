...
public class SubsonicCredentialStore
{
    ...
    public List<SubsonicCredential> Load()
    {
        ...
        {
            return _cached ??=
                AtomicJsonFile.Read(StorePath, SubsonicJsonContext.Default.SubsonicCredentialList, _logger)
    ...
    {
        await AtomicJsonFile.WriteAsync(StorePath, credentials, SubsonicJsonContext.Default.SubsonicCredentialList);
        lock (_cacheLock)
        {
            _cached = null;
        }
