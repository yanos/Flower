...
- `MOBILE-PLAN.md` — phased plan to get `Flower.iOS`/`Flower.Android` from scaffolding to actually runnable: per-platform workload/tooling gaps and missing native VLC package refs, per-platform data-import model (iOS private Documents folder vs. Android `MediaStore`), then the real blocker — no mobile UI exists yet (`App.axaml.cs`'s single-view branch, shared by both platforms, currently just shows the desktop `MainView`). Phase 3 (the UI) is explicitly one shared effort, not duplicated per platform.
